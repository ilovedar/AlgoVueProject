...
import {getAutowidth} from "../webix/export/topdf";
import {createCss} from "../webix/html";
import {protoUI, $$} from "../ui/core";
import {extend, bind, isUndefined, copy} from "../webix/helpers";
...
const api = {
	...
	$exportView:function(options){
		...
		if(options.sheets === true) 
			options.sheets  = this.getSheets().map(name => name.id || name);
		...
		let temp = [];
		const active = this._activeSheet;

		for(let i = 0; i<options.sheets.length; i++){
			const sheet = options.sheets[i];
			const id = sheet.id || sheet;
			const sheetOptions = sheet.options ? extend(copy(sheet.options), options) : copy(options);
			...
			if(mode == "pdf" && isUndefined(sheetOptions.textBefore))
				sheetOptions.textBefore = sheetOptions.name;

			const data = mode == "pdf" ? toPDF(this, sheetOptions) : toExcel(this, sheetOptions);
			if(sheetOptions.styles)
				data[0].styles = this._getExportStyles(sheetOptions);

			if(mode == "pdf" && options.autowidth)
